...
#define NGX_MEDIA_SRT_MAX_OUTPUTS 16

/* segment duration for the static HLS output */
#define NGX_MEDIA_SRT_HLS_TARGET 6000

/*
 * Per-stream outputs: the program feed is drained on the selection tick, muxed
 * into one burst and handed to HLS and to the PROGRAM recording; the ISO tap
 * muxes a named source before the selector and the RAW tap records transport
...
static char *ngx_media_srt_priority_cmd(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static ngx_media_policy_t *ngx_media_srt_policy(void);
static ngx_media_srt_outputs_t *ngx_media_srt_outputs_get(
    ngx_media_stream_t *stream, ngx_log_t *log);
static void ngx_media_srt_outputs_drain(ngx_media_srt_outputs_t *out,
    ngx_log_t *log);
static void ngx_media_srt_outputs_stop(ngx_media_srt_outputs_t *out);
static void ngx_media_srt_iso_frame(ngx_media_srt_slot_t *session,
...
static ngx_media_srt_outputs_t   ngx_media_srt_outputs[
    NGX_MEDIA_SRT_MAX_OUTPUTS];
static ngx_media_record_t        ngx_media_srt_raw;
static ngx_uint_t                ngx_media_srt_raw_ready;
...
ngx_media_srt_sink_frame(void *ctx, const ngx_media_frame_t *frame)
{
    ...
    if (session->stream != NULL && session->source != NULL) {
        ...
        ngx_media_srt_iso_frame(session, frame);
...
{
    ngx_media_policy_t  *policy = ngx_media_srt_policy();
    ...
    if (registry != NULL) {
             ...
             q = q->next)
        {
            ...
            if (ngx_media_srt_policy() != NULL) {
                ...
                {
                    ngx_media_srt_outputs_t  *out;

                    out = ngx_media_srt_outputs_get(stream, ev->log);

                    if (out != NULL) {
                        ngx_media_srt_outputs_drain(out, ev->log);
                    }
                }
            }
...
static ngx_media_srt_outputs_t *
ngx_media_srt_outputs_get(ngx_media_stream_t *stream, ngx_log_t *log)
{
    ngx_media_policy_t             *policy = ngx_media_srt_policy();
    ngx_media_srt_outputs_t        *out = NULL;
    ...
    for (i = 0; i < NGX_MEDIA_SRT_MAX_OUTPUTS; i++) {
        if (ngx_media_srt_outputs[i].used
            && ngx_media_srt_outputs[i].stream == stream)
        {
            return &ngx_media_srt_outputs[i];
        }
    }

    for (i = 0; i < NGX_MEDIA_SRT_MAX_OUTPUTS; i++) {
        if (!ngx_media_srt_outputs[i].used) {
            out = &ngx_media_srt_outputs[i];
            break;
        }
    }
    ...
    ngx_memzero(out, sizeof(ngx_media_srt_outputs_t));
    ...
    if (policy->hls_path.len > 0) {
        ...
        hls_conf.target_duration = NGX_MEDIA_SRT_HLS_TARGET;
        hls_conf.min_duration = NGX_MEDIA_SRT_HLS_TARGET / 2;
        hls_conf.max_duration = NGX_MEDIA_SRT_HLS_TARGET * 2;
...
static void
ngx_media_srt_outputs_flush(ngx_media_srt_outputs_t *out)
{
    size_t  len;
...
static void
ngx_media_srt_iso_flush(ngx_media_srt_outputs_t *out)
{
    size_t  len;
...
static void
ngx_media_srt_outputs_drain(ngx_media_srt_outputs_t *out, ngx_log_t *log)
{
    ...
    for ( ;; ) {
        ...
        for (i = 0; i < count; i++) {
            ...
            {
                ngx_media_srt_outputs_flush(out);
...
static void
ngx_media_srt_iso_frame(ngx_media_srt_slot_t *session,
    const ngx_media_frame_t *frame)
{
    ngx_media_policy_t       *policy = ngx_media_srt_policy();
    ngx_media_srt_outputs_t  *out;

    if (policy == NULL || policy->record_iso_path.len == 0
        || session->stream == NULL || session->source == NULL)
    {
        return;
    }

    if (policy->record_iso_source.len != session->source->id.len
        || ngx_memcmp(policy->record_iso_source.data, session->source->id.data,
                      session->source->id.len) != 0)
    {
        return;
    }

    out = ngx_media_srt_outputs_get(session->stream, ngx_cycle->log);

    if (out == NULL || !out->iso_ready) {
        return;
    }

    if (session->source->tracks != NULL
        && out->iso_tracks != session->source->tracks)
    {
        if (ngx_media_ts_mux_set_tracks(&out->iso_mux,
                                        session->source->tracks) == NGX_OK)
        {
            out->iso_tracks = session->source->tracks;
        }
    }
...
static void
ngx_media_srt_outputs_stop(ngx_media_srt_outputs_t *out)
{
    if (!out->used) {
        return;
    }
...
ngx_media_srt_init_process(ngx_cycle_t *cycle)
{
...

    {
        ...
        if (policy != NULL && policy->record_raw_path.len > 0) {
            ...
            if (ngx_media_record_init(&ngx_media_srt_raw, &rec_conf,
                                      cycle->log) == NGX_OK)
            {
                ngx_media_srt_raw_ready = 1;
...
ngx_media_srt_handler(ngx_event_t *ev)
{
    ...
    for ( ;; ) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (ngx_media_srt_raw_ready) {
                /* RAW tap: transport bytes before normalization */
                (void) ngx_media_record_append_bytes(&ngx_media_srt_raw,
                                                     chunks[i].data,
                                                     chunks[i].len);
            }
...
ngx_media_srt_exit_process(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < NGX_MEDIA_SRT_MAX_OUTPUTS; i++) {
        ngx_media_srt_outputs_stop(&ngx_media_srt_outputs[i]);
    }

    if (ngx_media_srt_raw_ready) {
        ngx_media_record_stop(&ngx_media_srt_raw);
        ngx_media_srt_raw_ready = 0;
    }
