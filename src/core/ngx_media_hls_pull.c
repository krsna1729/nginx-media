...
typedef struct ngx_media_hls_pull_s {
    ...
    /* segment names already fetched, so a refresh does not repeat them */
    u_char                   seen[NGX_MEDIA_HLS_PULL_NAMES][NGX_MEDIA_HLS_PULL_NAME_MAX];
    ngx_uint_t               nseen;
    ...
    size_t len)
{
    ...
    for (i = 0; i < pull->nseen; i++) {
        ...
        if (strlen((char *) pull->seen[i]) == len
            && ngx_memcmp(pull->seen[i], name, len) == 0)
        {
            return 1;
        }
    }

    if (pull->nseen < NGX_MEDIA_HLS_PULL_NAMES && len < NGX_MEDIA_HLS_PULL_NAME_MAX) {
        ngx_memcpy(pull->seen[pull->nseen], name, len);
        pull->seen[pull->nseen][len] = '\0';
        pull->nseen++;
    }
